...
from keras.layers import Conv2D, MaxPooling2D, ZeroPadding2D, UpSampling2D, AveragePooling2D
...
if not validateOnly:
    ...
    def train_for_n(nb_epoch, steps_per_epoch, batch_size, val_size, time_per_epoch, lr_idx):
        ...
        for e in tqdm(range(nb_epoch)):
            epoch_time_start = time.process_time()
            ...
            epoch_losses = {}
            total_epoch_time_elapsed = str(timedelta(seconds=(time.process_time() - epoch_time_start)))
            time_per_epoch.append(total_epoch_time_elapsed)

            #For each epoch, store intermediate results
            store_path = store_results('LWIR_to_RGB', loss_and_accuracy_e, params, {'transformer_Lab': transformer_Lab}, str(timedelta(seconds=(time.process_time() - time_start))), time_per_epoch, intermediate_results = True)
...
for lr_idx in range(len(lr)):
    ...
    total_time_elapsed = str(timedelta(seconds=(time.process_time() - time_start)))
